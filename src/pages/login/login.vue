<template>
    ...
    <div class="login_bar">
        ...
        <div class="login_group clearfix">
            ...
            <div class="login" @click='loginmock'>{{logintext}}</div>
        </div>
  ...
  export default {
    ...
    methods:{
        ...
        loginbeifen(){
            ...
            let timestamp= Date.parse( new Date())+'';
            var param={
                password: this.loginForm.password,        //表单v-model的值
                username: this.loginForm.username, 
                code:this.loginForm.code,
                timestamp:timestamp,
            }
            var asku=API_HZ+"login"+askUrl(param);
            var url = API_HZ+"login"+'?'+'username='+this.loginForm.username+'&password='+this.loginForm.password+'&sign='+sign+'&timestamp='+timestamp+'&code='+this.loginForm.code;
            var instance = axios.create();
            var bigheader='bigdata_'+timestamp;
            instance.defaults.headers.common['bigdata'] = To_md5(bigheader);
            // instance.defaults.headers.common['bigdata'] = To_md5('bigdata_1498578818000')
            instance.get(asku)
                .then((data ) => {
                    this.logintext='登录中'; 
        ...
        logisn(){
            ...
            var sign=paramToSign(this.loginForm)
            var url = API_HZ+"login"+'?'+'username='+this.loginForm.username+'&password='+this.loginForm.password+'&sign='+sign+'&timestamp='+timestamp+'&code='+this.loginForm.code;
            var instance = axios.create();
            var bigheader='bigdata_'+timestamp;
            instance.defaults.headers.common['bigdata'] = To_md5(bigheader);
